...
const PageNav = ({ isPageMenu, setIsPageMenu, classes }) => {
  ...
  return (
    <div
      className={`${
        isPageMenu ? 'visible' : 'hidden'
      } transition-all duration-700 grid grid-rows-[auto,1fr] min-h-screen w-full sec-x-padding fixed top-0 left-0 bg-body-main ${classes}`}
    >
      {/* nav controls */}
      <div className='screen-max-width w-full py-4 flex items-center justify-between mix-blend-difference'>
        <div>
          <NavLogo
            onClick={() => {
